# Prompt for ideas-34.json

**Save ChatGPT reply as:** test/project-ideas/ideas-34.json
...
| product_name | Smart IV Drip Rate Monitoring and Alert System |
...
- highlighttitle: A hospital-assist device that monitors intravenous drip rate and fluid level in real time, alerting nursing staff before a bottle runs empty or the flow rate drifts from the prescribed value.
- prodtags: ESP32, Arduino IDE, IR drop-counter sensor, load cell, LCD/OLED display, buzzer, Wi-Fi, IoT dashboard, Hospitals, ICUs, home healthcare
- existing short text: Project Name Smart IV Drip Rate Monitoring and Alert System Description A hospital-assist device that monitors intravenous drip rate and fluid level in real time, alerting nursing staff before a bottle runs empty or the flow rate drifts from the prescribed value. Technologies ESP32, Arduino IDE, IR drop-counter sensor, load cell, LCD/OLED display, buzzer, Wi-Fi, IoT dashboard Applications Hospitals, ICUs, home healthcare Report Contents Components List (BOM: Bill of Material) Block Diagram Flow Chart Components : Name, Images, Details Circuit Diagram Problem Statement Abstract Introduction Methodology Challenges and Solutions Performance Analysis Advantages Limitation Application Future Scope Conclusion Output Images Project Deliverables Project Hardware Project Report Project Simulation
...
- Be specific to **Smart IV Drip Rate Monitoring and Alert System** — avoid copy-paste generic IoT fluff.
- Include prodid=78 and category_slug="hardware" and product_name exactly as given.

After you produce the JSON, I will save it as ideas-34.json.
